...
export class AlunoService {
  ...
  private alunosCollection = collection(this.firestore, 'alunos');
  ...
  buscarPorNomeExato(nome: string) {
    ...
    const filtro = query(this.alunosCollection, where('nome', '==', nome));
  ...
  buscarPorNomeQueComeceCom(nome: string) {
    ...
    const filtro = query(this.alunosCollection,
      where('nome', '>=', nome),
      where('nome', '<', nome + '\uf8ff'));
    return collectionData(filtro, { idField: 'id' }) as Observable<Aluno[]>;
  }
